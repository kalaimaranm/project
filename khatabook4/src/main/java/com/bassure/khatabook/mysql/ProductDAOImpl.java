...
public class ProductDAOImpl implements ProductDAO {
    ...
    public boolean addProduct(Product product) throws ClassNotFoundException, SQLException {
        try (Connection connection = DbConnection.getConnection(); PreparedStatement ps = connection.prepareStatement("update product set cost_per_unit = ?,profit_per_unit = ?,weight = ?,date_time = ?,quantity = ? where id = ?");) {
    ...
    public boolean updateProduct(Product product) throws SQLException, ClassNotFoundException {
        ...
        try (Connection connection = DbConnection.getConnection(); PreparedStatement preparedStatement = connection.prepareStatement("update product set name = ?,cost_per_unit = ?,profit_per_unit = ?,weight = ?,quantity = quantity + ? where id = ?")) {
...
//                product = Arrays.copyOf(product, product.length + 1);
                products.put(rs.getInt(1), new Product(rs.getInt(1),
                        ...
                        rs.getString(7),
                        rs.getBoolean(8)));
//                product[product.length - 1] = new Product(rs.getInt(1),
//                        rs.getString(2),
//                        rs.getDouble(3),
//                        rs.getDouble(4),
//                        rs.getString(5),
...
// stoped here just do the delete product and add new functionality to front end 
...
//            if (Objects.nonNull(product) && product.isStatus()) {
//                return b == 1;
//            }
//            this methods not working
    ...
    public boolean productEntry(Order order) throws SQLException, ClassNotFoundException {
        try (Connection connection = DbConnection.getConnection(); 
            PreparedStatement ps = connection.prepareStatement("select quantity from product where id = ?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE); PreparedStatement ps2 = connection.prepareStatement("update product set quantity = ? where id = ?");) {
            int i = 0;
