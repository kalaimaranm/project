...
//    delete product then do the update product and customer 
    ...
    public int generateID(String name) throws SQLException, ClassNotFoundException {
        try (Connection connection = DbConnection.getConnection(); PreparedStatement ps = connection.prepareStatement("insert into product (name) values (?)"); PreparedStatement ps2 = connection.prepareStatement("select * from product where name = ?");) {
            ps.setString(1, name);
            ps.execute();
