...
//            return ps.execute();  Its not working?
    ...
    @Override  // validate customer status live,have debt?
    public boolean payDebts(double amount, int id) throws SQLException, ClassNotFoundException {
        try (Connection connection = DbConnection.getConnection();) {
            PreparedStatement ps = connection.prepareStatement("update customer set debt_balance = debt_balance - ? where id = ?");
    ...
    @Override
    public boolean updateCustomerBalance(int customerId, double amount) throws SQLException, ClassNotFoundException {
        try (Connection connection = DbConnection.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement("update customer set debt_balance = debt_balance + ? where id = ?")) {
            preparedStatement.setDouble(1, amount);
            preparedStatement.setInt(2, customerId);
            System.out.println("inside updateCustoemr balance: "+amount);
    ...
    public int getCustomerIdByPhoneNo(String phone) throws SQLException, ClassNotFoundException {
        try (Connection connection = DbConnection.getConnection();
              PreparedStatement ps = connection.prepareStatement("select id from customer where phone=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);) {
            ps.setString(1, phone);
            ResultSet rs = ps.executeQuery();
    ...
    public Customer getCustomerByPhoneNo(String phone) throws SQLException, ClassNotFoundException {
        ...
        try (Connection connection = DbConnection.getConnection(); 
            ...
            return rs.absolute(1) ? rs.getBoolean(7) ? customer = new Customer(rs.getInt(1),
                    ...
                    rs.getDouble(6),
                    rs.getBoolean(7),
                    khatabookDAO.getAddress().getAddress(rs.getInt(1)))
                    : customer
                    : customer;
        } catch (SQLException sql) {
            sql.printStackTrace();
    ...
    public Customer getCustomerById(int id) throws SQLException, ClassNotFoundException {
        Customer customer = null;
        try (Connection connection = DbConnection.getConnection(); PreparedStatement ps = connection.prepareStatement("select * from customer where id = ?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);) {
